...
|---|---|---|---|
| primary | **強い警告のみ**（滅多に使わない） | 本当に危機感を伝えたい1箇所だけ | 通常の課題・Before（neutralを使う）|
| secondary | メインカラー（最頻出） | 提案・目標・並列カード・フロー全般 | — |
| accent | 遷移・変化 | 矢印のみ | メインのボックス（装飾過多になる）|
| neutral | 現状・補足・サブ情報 | Before・課題・背景的要素 | 強調したい要素 |

### 状況別カラー選択ガイド

```
並列（3つとも同等価値）→ 全部 secondary
Before/After（対比）   → neutral → accent(矢印) → secondary
段階・フェーズ         → 全部 secondary（統一感優先）
階層（親子）           → neutral（親）+ secondary（子）
課題の列挙             → 全部 neutral（穏やかに示す）
本当に危機感が必要     → primary は1スライドに1箇所まで（滅多に使わない）
```
...
| B: テキスト＋図解 | あり（上） | あり（下） | なし | フロー・遷移型 |
| C: 全幅図解 | なし | あり（全幅） | なし | カード型・分類型 |
...
### content スライドの原則
- objects を選んだスライドには images を入れない（衝突回避の最良手段は共存させないこと）
...
#### 画像の再利用（統一感の鉄則）
- 表紙(title)の画像を agenda / chapter_photo / end で再利用して統一感を出す
...
| `content` | 主張・証拠 | A/B/Cパターンから1つ選ぶ。objects と images の混在禁止 |
...
- **objects + images を同一スライドに混在**: レイアウト崩壊の最大原因。必ずパターンA/B/Cから選ぶ
...
- **primary色の多用**: primary（赤系）は危機感の演出用。通常の課題には neutral を使う
...
| 状況 | 悪い例 | 良い例 |
|---|---|---|
| 課題 vs 解決 | ほぼ同じ色（微妙な差） | neutral（課題）vs secondary（解決）|
| 強調 vs 補足 | height差が微妙 | height: 0.8 vs 1.2（面積で差をつける）|
| 現在 vs 完了 | 全部同じ色 | neutral（現在）vs secondary（完了）|
...
- **同じ役割のボックスは同じスタイル**: 全スライドで「課題ボックス = neutral, height=0.8, font_size=12」を一貫
...
| **Before/After** | 現状課題→改善後の対比 | neutral→accent矢印→secondary |
...
## 11. ビジュアル要素の設計判断（AIデザイナーとしての意思決定）

Tier 2 展開時、各スライドに対して**デザイナーとして**以下を判断する。

### objects / images / 両方なし の選択フロー

```
スライドの主張は何か？
  ├→ 構造・フロー・比較を示す → objects のみ（画像不要）
  │   例: Before/After、ステップ図、並列カード、階層図
  ├→ 概念・ビジョン・雰囲気を伝える → image のみ（objects不要）
  │   例: 表紙、未来ビジョン、業界イメージ
  ├→ 数値・ファクトの羅列 → body のみ（objects/images 不要）
  │   例: 要件一覧、スケジュール表
  └→ 迷ったら → objects のみを選ぶ（安全で情報密度が高い）
```

### 絶対ルール
- **content スライドでは objects か images のどちらか一方を選ぶ**（混在はレイアウト崩壊の原因）
- objects がメッセージの核を担う場合、images は「飾り」でしかないので不要
- images は「言葉や図では伝わらない視覚的概念」が必要な場合にだけ使う
- title / agenda / chapter_photo / end はテンプレートの画像エリアに自動配置で統一感を出す

### テンプレート互換性の考慮
- 同じ Tier 2 ファイルが複数テンプレートで使われる可能性がある
- テンプレートによって content_area が異なるため、objects は中央配置を基本とする
- objects の中央配置計算: `left_start = (slide_width - group_width) / 2`
- `supports_image: false` のレイアウトでは画像が自動スキップされるが、設計時点で判断する方が品質が高い

---

## 12. テキスト品質基準（コンサルタント水準の文章）

### 聴衆を意識した言語選択
- **コード用語・内部用語をスライドに書かない**
  - NG: `skills/`, `Tier 2`, `outline.json`, `Read ツール`, `トークン`
  - OK: 「専門知識」「詳細設計」「構成案」「判断力」
- ファイル名をオブジェクトラベルに使わない（日本語の意味に変換する）
  - NG: `design_principles` → OK: 「デザイン原則」
  - NG: `slides/YYYYMMDD_xxx/` → OK: 「プロジェクト管理」
- 聴衆が技術者でない場合、技術実装の詳細ではなく**ビジネス価値**で表現する

### 箇条書きの品質
- 1つの箇条書きは **30〜50文字**（長すぎず短すぎず）
- 文末は **体言止め or 動詞終止** で統一（混在させない）
- 各項目の **粒度を揃える**（抽象↔具体がバラバラにならない）
- 具体的な数値やファクトがあれば積極的に入れる（「3〜5時間」「半分以上」等）

### タイトル・サブタイトルの品質
- タイトル: **20〜35文字**、スライドの結論を端的に示す
- サブタイトル: **40〜70文字**、タイトルを補足する1文（根拠・方法・効果のいずれか）
- 両方とも **聴衆が初見で理解できる言葉** を使う

---

## 13. テキスト量と余白バランス（サムネイル診断の基準）

### 余白3割の原則
スライド全体の面積のうち、**約30%が余白**であるのが適切。

```
余白が多すぎる（50%以上）→ 情報が薄い、説得力不足
  → body の箇条書きを増やす、objects を追加する
余白が少なすぎる（20%以下）→ 詰め込みすぎ、読みにくい
  → 箇条書きを減らす、objects を簡略化する、スライドを分割する
適切（30%前後）→ 視線の休憩があり、要素が際立つ
```

### サムネイル診断での調整
1. サムネイルを見て「スカスカ」に感じたら → body 追加・objects 追加
2. サムネイルを見て「ぎっしり」に感じたら → 箇条書き削減・スライド分割
3. 左右のバランスも確認（片側に偏っていないか）
...
### いつ出典が必要か
- 外部の資料・Webサイトからデータや情報を引用した場合
- 統計データ、調査結果、他社事例を記載した場合
- 「〜によると」「〜の調査では」と書く場合

### 出典の配置方法
スライド下部に `text` オブジェクトとして配置する。

```json
{"type": "text", "text": "出典: 総務省「令和6年版 情報通信白書」p.42",
 "left": 0.5, "top": 6.8, "width": 12.0, "height": 0.3,
 "font_size": 9, "font_color": "808080"}
```

### 出典のフォーマット
| 種類 | 書式 |
|---|---|
| 書籍・報告書 | `出典: 著者名「書籍名」出版社, 年, p.XX` |
| Webサイト | `出典: サイト名「ページタイトル」URL（参照日）` |
| 社内資料 | `出典: 部門名「資料名」YYYY年MM月` |

### ルール
- フォントサイズ: **9pt**（本文より明確に小さく）
- フォントカラー: **808080**（グレー、目立たせない）
- 位置: スライド下端（`top: 6.8` 付近、フッター直上）
- 1スライドに複数出典がある場合は改行で列挙
- 出典がないスライドには配置しない（不要な装飾を避ける）

---

## 15. セルフチェックリスト（Tier 2 生成前に確認）

**基本原則（全要素共通）**
- [ ] ボックスの height は 1行=0.8、2行=1.2 以上か？
- [ ] 矢印の top = `ボックスtop + height/2 - 矢印height/2` で計算したか？
- [ ] 並列ボックスの top は全て揃っているか？
- [ ] フォントサイズは 10pt 以上か（2行テキストは11pt推奨）？
- [ ] 白文字を使う場合、背景が十分暗いか？

**レイアウト（配置）**
- [ ] content スライドで objects と images を混在させていないか？（A/B/Cパターンを選択したか）
- [ ] objects は水平中央配置か？（`left_start = (slide_width - group_width) / 2`）
- [ ] objects の right端（left+width）が content_area.right 以内か？
- [ ] N個のボックスを均等幅で配置しているか（計算式を使ったか）？
- [ ] 右下や左下に大きな空白の象限がないか？

**デザイナー判断（ビジュアル要素の選択）**
- [ ] このスライドに objects / images / なし のどれが最適か判断したか？
- [ ] objects で構造を示せるなら images を入れていないか？
- [ ] title/agenda/chapter_photo/end の画像はテンプレート自動配置を使っているか？
- [ ] fill_color に primary（赤系）を安易に使っていないか？（neutral で十分な場面が多い）

**テキスト量・余白バランス**
- [ ] サムネイルで見て余白が約3割あるか？（詰め込みすぎ・スカスカを回避）
- [ ] body の箇条書きが5行以上ならフォントサイズや内容を調整したか？
- [ ] objects を多数並べて窮屈になっていないか？

**出典・引用**
- [ ] 外部データ・調査結果を引用した場合、出典テキストを配置したか？
- [ ] 出典は 9pt / 808080 / top:6.8 付近に配置しているか？
- [ ] 出典がないスライドに不要な出典欄を入れていないか？

**テキスト品質（コンサルタント水準）**
- [ ] コード用語・ファイル名・内部用語がスライドに残っていないか？
- [ ] objects ラベルが日本語の意味になっているか？（ファイル名 NG）
- [ ] 箇条書きの粒度と文末形式が揃っているか？
- [ ] タイトル 20〜35文字、サブタイトル 40〜70文字の範囲か？
- [ ] 聴衆が初見で理解できる言葉を使っているか？
