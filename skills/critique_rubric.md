...
- [ ] フォントサイズの使い分けが明確か（見出し > 本文 > 補足、の階層が崩れていないか）

#### C. カラー・コントラスト（SX Documentation Master 準拠）
- [ ] fill_color が2種類以内か（矢印の橙は除く）
- [ ] 対比要素（課題 vs 解決）が neutral vs secondary で明確に区別されているか
- [ ] 白文字が使われている場合、背景が十分暗いか
- [ ] **基本3色**: 白・黒・Dynamic Orange（SXロゴ橙）を基調としているか
- [ ] **強調色は Dynamic Orange のみ**。色を追加する場合はテンプレートのカラーパレットから左→右の順に選択
- [ ] グレースケールでの濃淡差が確保されているか（橙を除く差別化はグレースケールで）
...
#### D2. 図表・チャートの品質
- [ ] チャート・グラフには「何を伝えたいか」のメッセージが明記されているか
- [ ] 強調箇所には Dynamic Orange、その他の差別化にはグレースケールを使用しているか

#### D3. 出典の記載
- [ ] 外部データ・統計・事例を記載するスライドに出典が記載されているか
- [ ] 出典フォーマット: `出所: (出典元, ページ, 著者 / サイト名, URL, 運営者)`
