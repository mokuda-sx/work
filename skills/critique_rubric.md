...
- [ ] content スライドに `objects` か `images` のいずれかがあるか（図解ゼロは禁止）
...
- [ ] 対比要素（課題 vs 解決）が赤 vs 青で明確に区別されているか
...
#### E. パターン適合確認
- body + objects + images の3要素が同時に使われていないか
- 使われているパターンはA/B/C/Dのどれか？
...
| 優先度 | 症状 | 対応 |
|---|---|---|
| **S（即修正）** | objects が左端に偏り右半分が空白、3要素共存で崩壊 | その場で Tier 2 Edit |
...
| `content` | パターンA/B/C/Dのどれか、objects偏りがないか | 上記チェックリストで確認 |
